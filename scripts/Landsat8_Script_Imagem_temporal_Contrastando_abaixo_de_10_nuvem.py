...
inicio = '2025-04'
fim = '2025-06'
bands = ['SR_B5', 'SR_B6', 'SR_B4']  # NIR, SWIR1, RED (Landsat 8)
...
def buscar_imagem(data_ini, data_fim, label):
    for limite in range(10, 101, 10):
        ...
        if nuvem is not None and nuvem < 10:
            try:
                ...
                if None not in min_vals and None not in max_vals:
                    ...
                    Map.addLayer(imagem, vis, f'{label}: {data_img} (contraste)')
                    print(f'{label} - {data_img} - nuvem {nuvem:.1f}% - contraste aplicado')
                    return
            except Exception as e:
                print(f'{label} - {data_img} - erro no contraste: {e}')

        # Caso não consiga aplicar contraste ou nuvem ≥ 10%
        vis = {'bands': bands, 'min': 0, 'max': 0.3, 'gamma': 1.0}
        Map.addLayer(imagem, vis, f'{label}: {data_img} (nuvem {nuvem if nuvem is not None else "?"}%)')
